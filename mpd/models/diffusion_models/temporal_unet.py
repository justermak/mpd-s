...
UNET_DIM_MULTS = {0: (1, 2, 4), 1: (1, 2, 4, 8)}
...
class EnvModel(nn.Module):
    def __init__(self, in_dim=16, out_dim=16, **kwargs):
        super().__init__()
        self.in_dim = in_dim
...
class TaskModel(nn.Module):
    def __init__(self, in_dim=16, out_dim=32, **kwargs):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
...
class ContextModel(nn.Module):
    def __init__(self, env_model=None, task_model=None, out_dim=32, **kwargs):
        super().__init__()
...
class PointUnet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            self.downs.append(
                nn.ModuleList([TemporalBlockMLP(dim_in, dim_out, cond_dim)])
            )
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            self.ups.append(
                nn.ModuleList([TemporalBlockMLP(dim_out * 2, dim_in, cond_dim)])
            )
